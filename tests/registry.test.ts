...
describe('tool registry shape', () => {
  it('exposes the Phase 1 tools plus Phase 2 through 6 tools', () => {
    const names = toolDefinitions.map((t) => t.name).sort();
    expect(names).toEqual(
      ['analyze_access_control', 'analyze_project', 'discover_routes', 'get_project_info', 'list_files', 'list_verification_cases', 'read_file', 'run_command', 'scan_project', 'search_files', 'verify_finding'].sort()
    );
  });
